...
t_scene	init_scene(void)
{
	...
	scene.ambient_light.color = (t_color){1, 1, 1};
	...
	scene.objects[0].origin = (t_point3){1, 1, -1};
	scene.objects[0].radius = 0.5;
	scene.objects[0].mat.albedo = (t_color){1, 0, 0};
	scene.objects[1].origin = (t_point3){0, -100.5, -1};
	scene.objects[1].radius = 100;
	scene.objects[1].mat.albedo = (t_color){1, 0, 0};
	scene.obj_count = 1;
	return (scene);
}
